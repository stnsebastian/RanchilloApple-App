// Ranchillo Apple - App Mobile-First optimizada para celulares Apple (iPhone / iPad / iOS Safari)
    ...
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', () => {
            navigator.serviceWorker.register('./sw.js')
                .then(reg => console.log('Service Worker Apple registrado con éxito:', reg.scope))
                .catch(err => console.error('Fallo al registrar Service Worker:', err));
        });
    }
}

// --- DETECCIÓN DE CELULARES APPLE (iPhone / iPad / iOS) ---
...
// --- CONFIGURACIÓN DE INSTALACIÓN APPLE ---
function configurarInstalacionApple() {
    const appleBanner = document.getElementById('appleBanner');
    if (!appleBanner) return;

    // Si está en un dispositivo Apple y no está instalada como PWA nativa, mostrar el banner sugerido
    if (esDispositivoApple() && !esStandalone()) {
        appleBanner.style.display = 'flex';
    }
}

function abrirGuiaApple() {
    vibrar(25);
    const modal = document.getElementById('appleInstallModal');
    if (modal) {
...
function cerrarBannerApple() {
    vibrar(15);
    const appleBanner = document.getElementById('appleBanner');
    if (appleBanner) {
        appleBanner.style.display = 'none';
    }
}
...
// --- CONTROLES DE BOTTOM SHEET ---
function abrirModalRegistro() {
    vibrar(25);
    const modal = document.getElementById('bottomSheetRegistro');
    if (modal) {
        modal.classList.add('active');
        document.body.style.overflow = 'hidden';
    }
...
// --- REPORTE COMPARTIDO (OPTIMIZADO CON iOS SHARE SHEET & WHATSAPP) ---
async function enviarWhatsApp() {
    ...
    let texto = `*🐾 RANCHILLO APPLE - REPORTE DE CAJA DIARIO 🍏*\n`;
    ...
    // Si el iPhone / iPad soporta Web Share nativo, ofrecer compartir directamente
    if (navigator.share && esDispositivoApple()) {
        try {
            await navigator.share({
                title: 'Reporte Diario Ranchillo Apple',
                text: texto
            });
            showToast('✅ Reporte compartido');
            return;
        } catch (e) {
            // Si cancela o prefiere abrir WhatsApp, caemos al enlace clásico
        }
    }
...
// --- CONTROL DEL TEMA VISUAL Y PALETAS ---
const TEMAS_PALETA = {
    azul: {
...
// --- BASE DE DATOS PREDETERMINADA (122 PRODUCTOS RANCHILLO) ---
...
window.abrirGuiaApple = abrirGuiaApple;
window.cerrarGuiaApple = cerrarGuiaApple;
window.cerrarBannerApple = cerrarBannerApple;
